function varargout = analyzeWhiskersSetup_gui_v1(varargin)
%ANALYZEWHISKERSSETUP_GUI_V1 MATLAB code file for analyzeWhiskersSetup_gui_v1.fig
%      ANALYZEWHISKERSSETUP_GUI_V1, by itself, creates a new ANALYZEWHISKERSSETUP_GUI_V1 or raises the existing
%      singleton*.
%
%      H = ANALYZEWHISKERSSETUP_GUI_V1 returns the handle to a new ANALYZEWHISKERSSETUP_GUI_V1 or the handle to
%      the existing singleton*.
%
%      ANALYZEWHISKERSSETUP_GUI_V1('Property','Value',...) creates a new ANALYZEWHISKERSSETUP_GUI_V1 using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to analyzeWhiskersSetup_gui_v1_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      ANALYZEWHISKERSSETUP_GUI_V1('CALLBACK') and ANALYZEWHISKERSSETUP_GUI_V1('CALLBACK',hObject,...) call the
%      local function named CALLBACK in ANALYZEWHISKERSSETUP_GUI_V1.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help analyzeWhiskersSetup_gui_v1

% Last Modified by GUIDE v2.5 14-Dec-2017 14:56:04

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @analyzeWhiskersSetup_gui_v1_OpeningFcn, ...
    'gui_OutputFcn',  @analyzeWhiskersSetup_gui_v1_OutputFcn, ...
    'gui_LayoutFcn',  [], ...
    'gui_Callback',   []);
...
% --- Executes just before analyzeWhiskersSetup_gui_v1 is made visible.
function analyzeWhiskersSetup_gui_v1_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for analyzeWhiskersSetup_gui_v1
handles.output = hObject;
handles.runTrace = 1;
% Update handles structure
guidata(hObject, handles);

% UIWAIT makes analyzeWhiskersSetup_gui_v1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = analyzeWhiskersSetup_gui_v1_OutputFcn(hObject, eventdata, handles)
...
axes(ax1),yyaxis left,ax1.YColor = 'k';
yyaxis right,ax1.YColor = 'r';
...
while (stopStatus == 0) && hasFrame(vidobj)%(currFrames <= handles.testFrames)
    ...
    yyaxis left, line(frameList,plotAngle,'Color','k'),ylabel('median angle')
    yyaxis right,line(frameList,plotsumDiff,'Color','r'),ylabel('sum frame diff')
